...
function Pixrem (rootvalue, options) {
  if (typeof options === 'undefined' && typeof rootvalue === 'object') {
    options = rootvalue;
    rootvalue = undefined;
  }
  _remgex = /(\d*\.?\d+)rem/ig;
  ...
  _options.rootvalue = (options.rootvalue !== undefined) ? options.rootvalue : _rootvalue;
}
      ...
      if (/^(html|:root)$/.test(rule.selectors)) {
        rule.eachDecl(function (decl) {
          if (decl.prop === 'font-size') {
            _options.rootvalue = decl.value;
          } else if (decl.prop === 'font' && decl.value.match(/\d/)) {
            _options.rootvalue = decl.value.match(/.*?([\d\.]*(em|px|rem|%|pt|pc))/)[1];
          }
        });
      }
    });
      ...
      if (value.indexOf('rem') !== -1) {
        ...
        if ( isSpecialCaseIE9_10 || isUseless ) {
          ...
          value = value.replace(_remgex, function ($1) {
            ...
            return Math.floor(parseFloat($1) * toPx(_options.rootvalue)) + 'px';
          });
...
function toPx (value) {
  ...
  var parts = /^\s*(\d*\.?\d+)([a-zA-Z%]*)\s*$/.exec(value);
  if (parts !== null) {
    var number = parts[1];
    var unit   = parts[2];
    ...
    throw new Error('Root font-size is invalid: ' + JSON.stringify(value));
  }
}
